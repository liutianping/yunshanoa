...
{
    public partial class DocumentExamine : System.Web.UI.Page
    {
        ...
        {
            gvDocumentTemplate.DataSource = new YunShanOA.BusinessLogic.DocumentManager.DocumentManager().getListByStatus(1);
        ...
        {
            YunShanOA.Workflow.DocumentWF.ReviewQicao info = new Workflow.DocumentWF.ReviewQicao();
            ...
            try
            {
                DocumentWorkFlowProcess.RunReviewQicao(Guid.Parse(this.txtWFID.Text.ToString()), info);
                this.btnSubmit.Enabled = false;
                this.lbMsg.Text = "已经成功提交";
                gvDateBind();
            }
            catch (Exception)
            {
